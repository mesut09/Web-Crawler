...
# hata verince uygulamanın sapmaması için try-except bloklarını kullandım.
neededdata=[]
for i in jsons:
    try:
...
#boş değer geldiğinde hata vermemesi adına 0 değeri döndürecek
X = X.fillna(0)
y = y.fillna(0)
...
# Değerlendirme prosedürümüzü tanımladık
cv = RepeatedKFold(n_splits=10, n_repeats=3, random_state=1)
# Modeli değerlendirdik ve sonuçları topladık
n_scores = cross_val_score(model, X, y, scoring='neg_mean_absolute_error', cv=cv, n_jobs=-1)
# Puanları pozitif olmasına zorladık
n_scores = absolute(n_scores)
# Analizin sonuçlarını göster
print('MAE: %.3f (%.3f)' % (mean(n_scores), std(n_scores)))
accuracyKnn = mean(n_scores)



# multi regression için linear regression kullandık
from sklearn.datasets import make_regression
from sklearn.linear_model import LinearRegression
# datasetimizi oluşturduk
X, y = make_regression(n_samples=500, n_features=3, n_informative=5, n_targets=2, random_state=1, noise=0.5)
# Modelimizi tanımladık
model = LinearRegression()
# em uygun modeli seçmesini istedik
model.fit(X, y)
